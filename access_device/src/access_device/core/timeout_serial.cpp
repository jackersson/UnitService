...
namespace access_device
{
	TimeoutSerial::TimeoutSerial() : io(), port(io), timer(io),
		timeout(posix_time::seconds(0)), result()
	{}
	...
	bool TimeoutSerial::is_open() const
	{
		return port.is_open();
	}
	...
	void TimeoutSerial::set_timeout(const posix_time::time_duration& t)
	{
		timeout = t;
	}
	...
	void TimeoutSerial::write_string(const std::string& s)
	{
